...
with c1:
    fig_cost = go.Figure(data=[
        go.Bar(name='Estimated', x=['Total Cost', 'FO Cost', 'DO Cost'], y=[est_total_cost, est_fo_cost, est_do_cost], marker_color='#9467bd', text=[f"${v:,.0f}" for v in [est_total_cost, est_fo_cost, est_do_cost]], textposition='auto'),
        go.Bar(name='Actual', x=['Total Cost', 'FO Cost', 'DO Cost'], y=[act_total_cost, act_fo_cost, act_do_cost], marker_color='#e377c2', text=[f"${v:,.0f}" for v in [act_total_cost, act_fo_cost, act_do_cost]], textposition='auto')
    ])
...
with c2:
    fig_fuel = go.Figure(data=[
        go.Bar(name='Estimated', x=['Total FO', 'Total DO'], y=[est_tot_fo, est_tot_do], marker_color='#1f77b4', text=[f"{v:,.0f}" for v in [est_tot_fo, est_tot_do]], textposition='auto'),
        go.Bar(name='Actual', x=['Total FO', 'Total DO'], y=[act_tot_fo, act_tot_do], marker_color='#d62728', text=[f"{v:,.0f}" for v in [act_tot_fo, act_tot_do]], textposition='auto')
    ])
...
with c3:
    fig_time = go.Figure(data=[
        go.Bar(name='Estimated', x=['Total Days', 'Sea Days', 'Work Days', 'Idle Days'], y=[est_tot_days, est_sea_days, est_work_days, est_idle_days], marker_color='#2ca02c', text=[f"{v:.1f}" for v in [est_tot_days, est_sea_days, est_work_days, est_idle_days]], textposition='auto'),
        go.Bar(name='Actual', x=['Total Days', 'Sea Days', 'Work Days', 'Idle Days'], y=[act_tot_days, act_sea_days, act_work_days, act_idle_days], marker_color='#ff7f0e', text=[f"{v:.1f}" for v in [act_tot_days, act_sea_days, act_work_days, act_idle_days]], textposition='auto')
    ])
...
with c4:
    fig_eff_sea = go.Figure(data=[
        go.Bar(name='Estimated', x=['Laden MT/NM', 'Ballast MT/NM'], y=[est_lad_mt_nm, est_bal_mt_nm], marker_color='#17becf', text=[f"{v:.4f}" for v in [est_lad_mt_nm, est_bal_mt_nm]], textposition='auto'),
        go.Bar(name='Actual', x=['Laden MT/NM', 'Ballast MT/NM'], y=[act_lad_mt_nm, act_bal_mt_nm], marker_color='#bcbd22', text=[f"{v:.4f}" for v in [act_lad_mt_nm, act_bal_mt_nm]], textposition='auto')
    ])
...
with c5:
    fig_eff_port = go.Figure(data=[
        go.Bar(name='Estimated', x=['Work MT/Day', 'Idle MT/Day'], y=[est_work_mt_day, est_idle_mt_day], marker_color='#17becf', text=[f"{v:.2f}" for v in [est_work_mt_day, est_idle_mt_day]], textposition='auto'),
        go.Bar(name='Actual', x=['Work MT/Day', 'Idle MT/Day'], y=[act_work_mt_day, act_idle_mt_day], marker_color='#bcbd22', text=[f"{v:.2f}" for v in [act_work_mt_day, act_idle_mt_day]], textposition='auto')
    ])
...
def generate_voyage_pdf():
    ...
    try:
        ...
        rects1_0 = ax0.bar(x_cost - width/2, est_cost_vals, width, label='Estimated', color='#9467bd')
        rects2_0 = ax0.bar(x_cost + width/2, act_cost_vals, width, label='Actual', color='#e377c2')
        
        effects = [patheffects.withStroke(linewidth=3, foreground="black")]
        ax0.bar_label(rects1_0, padding=3, fmt='${:,.0f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ax0.bar_label(rects2_0, padding=3, fmt='${:,.0f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ...
        rects1 = ax.bar(x_fuel - width/2, est_fuel_vals, width, label='Estimated', color='#1f77b4')
        rects2 = ax.bar(x_fuel + width/2, act_fuel_vals, width, label='Actual', color='#d62728')
        
        effects = [patheffects.withStroke(linewidth=3, foreground="black")]
        ax.bar_label(rects1, padding=3, fmt='{:,.0f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ax.bar_label(rects2, padding=3, fmt='{:,.0f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ...
        rects1_2 = ax2.bar(x_time - width/2, est_time_vals, width, label='Estimated', color='#2ca02c')
        rects2_2 = ax2.bar(x_time + width/2, act_time_vals, width, label='Actual', color='#ff7f0e')
        
        effects = [patheffects.withStroke(linewidth=3, foreground="black")]
        ax2.bar_label(rects1_2, padding=3, fmt='{:.1f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ax2.bar_label(rects2_2, padding=3, fmt='{:.1f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ...
        rects1_3 = ax3.bar(x_eff_sea - width/2, est_eff_sea, width, label='Estimated', color='#17becf')
        rects2_3 = ax3.bar(x_eff_sea + width/2, act_eff_sea, width, label='Actual', color='#bcbd22')
        
        effects = [patheffects.withStroke(linewidth=3, foreground="black")]
        ax3.bar_label(rects1_3, padding=3, fmt='{:.4f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ax3.bar_label(rects2_3, padding=3, fmt='{:.4f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ...
        rects1_4 = ax4.bar(x_eff_port - width/2, est_eff_port, width, label='Estimated', color='#17becf')
        rects2_4 = ax4.bar(x_eff_port + width/2, act_eff_port, width, label='Actual', color='#bcbd22')
        
        effects = [patheffects.withStroke(linewidth=3, foreground="black")]
        ax4.bar_label(rects1_4, padding=3, fmt='{:.2f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ax4.bar_label(rects2_4, padding=3, fmt='{:.2f}', color='#FFFFE0', path_effects=effects, weight='bold')
        ...
        pdf.image(cost_img_path, x=10, y=20, w=190)
        pdf.image(fuel_img_path, x=10, y=130, w=190)
        ...
        # Page 4: Port Efficiency and Comments
        pdf.add_page()
    ...
    # --- ENERGY DEPARTMENT COMMENT IN PDF ---
    current_y = pdf.get_y()
    if current_y < 120:
        pdf.set_y(140)
    else:
